...
\begin{document}
  ...
  \date{2026\\[2pt]\small Version 1.3.3}
  \maketitle

  \begin{abstract}
    The O-series defines a canonical pair-level capacity observable and establishes that
    vertical non-injectivity does not change its observable rank.
    The further prescription $\bstar=1/(\dpair+\tfrac12)$, however, imports the
    changing-degree LPS growth equation into a fixed-degree Heisenberg cascade and has no
    native carrier there.
    The value $\dpair \approx 7.44$ extracted in O16 was based on a
    ...
    First, $\dpair(q)$ is extracted reproducibly and concentrates across pairs, confirming that it is a
    stable fixed-$q$ pair statistic rather than a block-level fluctuation.
    ...
    normalization correction, needed to bring the small-$q$ values into the
    ...
    For comparison only, applying the legacy reciprocal map
    $\bstar = 1/(\dpair + \tfrac12)$ produces the narrow interval $0.108$--$0.123$ across
    $q \in \{211, \dots, 601\}$.  This is a phenomenological numerical coincidence, not a
    Heisenberg capacity-to-rate inference.
  ...
  \section{Position of the Problem}
    ...
    Beau2026a25,Beau2026a26,Beau2026a27,Beau2026a28},
      ...
      O24 proves that vertical fibre structure does not perturb the observable rank entering
      $\dpair$~\cite{Beau2026a28}.  It does not supply the missing growth carrier needed to infer
      $\bstar$: the O4--O6 equation and the Heisenberg capacity observable live on different
      substrates~\cite{Beau2026sgn}.
    ...
    \subsection{Scope of O25}
      ...
      Throughout this paper, that window is retained as a historical phenomenological comparison;
      it is not evidence for the refuted Heisenberg capacity-to-rate transfer.
  ...
  \section{Setup and Protocol}
    ...
    \subsection{Pair protocol}
      ...
      \begin{enumerate}
        ...
        These reported exponents retain that frozen estimator convention.  Regressing against
        $\log n$, as in the paper-level power-law definition, lowers the fitted exponent by
        approximately $8$--$13\%$ on the relevant finite windows~\cite{Beau2026sgn}; this
        estimator effect does not alter the concentration results below.
      \end{enumerate}
    ...
    \subsection{Fitting window and BFS parameters}
      ...
      \begin{figure}[t]
        \centering
        \includegraphics[width=\textwidth]{../code/o25_fig1_sigma_pair}
  ...
  \section{Numerical Results}
    ...
    \subsection{Inter-pair concentration}
      ...
      This concentration establishes a stable fixed-$q$ pair statistic of the
      Weil representation on $\Heis$ and is compatible with the verticality theorem of
      O24~\cite{Beau2026a28}.  The theorem excludes rank changes induced by vertical fibre
      structure; it does not by itself prove that all remaining inter-pair variance is sampling
      noise or that the statistic is independent of~$q$.

      \begin{figure}[t]
        \centering
        \includegraphics[width=\textwidth]{../code/o25_fig2_distribution}
    ...
    \subsection{Extension to large primes ($q$ up to $601$)}
      ...
      \begin{figure}[t]
        \centering
        \includegraphics[width=\textwidth]{../code/o25_scaling_extended}
      ...
      As a legacy diagnostic, the numerical image
      $1/(\dpair + \tfrac12)$ stays in the narrow band $0.108$--$0.123$ across
      $q \in \{211, \dots, 601\}$ (Table~\ref{tab:extended}).  Since the reciprocal map is not
      natively derived on the Heisenberg substrate~\cite{Beau2026sgn}, this stability is not a
      determination of $\bstar$.
  ...
  \section{Structural Analysis: Why Extrapolation Fails}
    ...
    \subsection{Leading finite-window correction}
    ...
    \subsection{Corrected values are consistent with the admissible window}
      ...
      \begin{figure}[t]
        \centering
        \includegraphics[width=\textwidth]{../code/o25_fig3_convergence}
      ...
      \begin{figure}[t]
        \centering
        \includegraphics[width=\textwidth]{../code/o25_scaling}
        \caption{Convergence of $\dpair$ with $q$ from the extended O25 campaign
          ...
          historical reference value $7.44$.
          ...
          Green dashed: historical reference value $\delta_{\mathrm{pair}} \approx 7.44$.
  ...
  \section{The Central Open Direction: $n_1(q)/q$}
    ...
    From Table~\ref{tab:params},
    \begin{align*}
      n_1(q)/q &\in \{0.138, 0.131, 0.109, 0.086, 0.066\},\\
      q &\in \{29, 61, 101, 151, 211\}.
    \end{align*}
    ...
    \end{enumerate}
    Answering these questions would help constrain $\delta_\infty$
  ...
  \section{Discussion}
    ...
    \subsection{What O25 establishes}
      ...
      \begin{enumerate}
        ...
        $\dpair(q)$ is extracted reproducibly and the inter-pair variance decreases strongly with $q$.
        ...
        This is compatible with the rank-stability result of O24~\cite{Beau2026a28}.
        ...
        the candidate analytical variable to test is $n_1(q)/q$, not $q$ alone.
    ...
    \subsection{Relation to O24 and the transfer no-go}

      O24~\cite{Beau2026a28} established a fibre-side result: vertical non-injectivity preserves
      the observable rank.  That theorem remains intact.  It does not establish the full chain
      $c_{\mathrm{BI}} \to \dpair \to \bstar$ because the O4--O6 growth equation is native to a
      changing-degree LPS family, whereas $\dpair$ is measured on a fixed-degree Heisenberg
      cascade.  The missing cross-substrate identification is refuted for the two native
      realisations present in the frozen corpus~\cite{Beau2026sgn}.
      ...
      Applying the former reciprocal prescription to those measurements gives the narrow band
      $0.108$--$0.123$ (Table~\ref{tab:extended}), but only as a phenomenological diagnostic.
      The pair campaign closes its numerical concentration target; it does not close a native
  ...
  \section{Conclusion}
    ...
    The extraction of $\dpair(q)$ is robust and the inter-pair concentration is strong,
    establishing that $\dpair$ is a concentrated fixed-$q$ pair statistic of the admissibility
    programme.
    ...
    Applying the legacy reciprocal expression $1/(\dpair + \tfrac12)$ yields
    $0.108$--$0.123$ across $q \in \{211, \dots, 601\}$, but the Heisenberg
    capacity-to-rate transfer required to interpret this image as $\bstar$ is not natively
    available.  The numerical interval is therefore retained only as a phenomenological
    comparison~\cite{Beau2026sgn}.
  ...
  \section*{Data and code availability}
    ...
    \path{delta_pair_convergence_campaign.py} (resumable, live ETA), the convergence analysis
    \path{delta_pair_analysis.py} (knee-trimmed interior fit, $1/q$-vs-$1/\sqrt{q}$ scan,
    bootstrap), and the checkpoints \texttt{dpc\_q\{307,401,601\}.npz}, all in
    \path{simulation/spectral/o25}; the capped construction reuses
    \path{n1_resumable.build_bfs} and reproduces \path{compute_block_capacity} exactly.

  \section*{Acknowledgements}

    The author thanks the extended discussions within the Cosmochrony programme
    that shaped the identification of $n_1(q)/q$ as a candidate asymptotic variable.
